...
export function HMConditionCurrentToNode(device: FactoryDevice, channels: Channel[], conn: CCUConnectionInfo) {
    if (channels.length === 1) {
        createConditionCurrentNode(device, channels[0], conn);
    } 

}

function createConditionCurrentNode(device: FactoryDevice, channel: Channel, conn: CCUConnectionInfo) {
    ...
    // Set current state
    node.conditionCurrent = channel.readings[conditionChannel_Param_DecisionValue]?.value;
    ...
    device.events$.pipe(takeUntil(node.onDestroy$)).subscribe(
        {
            next: message => {
                if (message.deviceAddress === channel.definition.ADDRESS){
                    node.conditionCurrent=message.value;
